...
impl Renderer {
    ...
    pub fn render(
        ...
        _text_block_index: Option<usize>,
        _effect: TextShaderEffect,
        _stroke: Option<TextStrokeStyle>,
    ...
    fn render_text_block(
        ...
        let Some(text) = text_block.text.as_ref().cloned() else {
            return Ok(());
        };
        if text.is_empty() {
            return Ok(());
        };
        let normalized_text = normalize_translation_for_layout(&text);
        ...
        apply_default_font_families(&mut style.font_families, &normalized_text);
        ...
        let english_layout = english_layout_behavior(text_block, &normalized_text, writing_mode);
...
fn english_layout_behavior(
    ...
    let is_english_horizontal =
        writing_mode == WritingMode::Horizontal && is_latin_only(normalized_text);
