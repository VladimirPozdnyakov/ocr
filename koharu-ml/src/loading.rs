...
where
    ...
    E: Into<anyhow::Error>,
{
    // SAFETY: The safetensors file is expected to be a valid model weights file.
    // Memory-mapping is safe as long as the file exists and is not truncated
    // while in use. The file path comes from a resolved manifest and should
    // point to a valid safetensors file downloaded from HuggingFace.
