...
Форк [Koharu](https://github.com/mayocream/koharu) для команды [Lilith Team](https://t.me/+yq6z7BZcZ84xODJi)

OCR-приложение для манги, написанное на **Rust**.

Koharu (Lilith Edition) использует возможности машинного обучения для автоматизации распознавания текста в манге. Он объединяет возможности обнаружения объектов и OCR для извлечения текста из изображений.

Под капотом Koharu (Lilith Edition) использует [candle](https://github.com/huggingface/candle) для высокопроизводительного инференса. Все компоненты написаны на Rust, что обеспечивает безопасность и скорость.

> [!NOTE]
> Koharu (Lilith Edition) запускает свои визуальные модели **локально** на вашем компьютере. Koharu (Lilith Edition) не собирает пользовательские данные.
...
Koharu (Lilith Edition) работает как веб-приложение с серверной частью на Rust.
...
Koharu (Lilith Edition) экспортирует результаты OCR в текстовый файл.

## GPU-ускорение

CUDA поддерживаются для GPU-ускорения, что значительно улучшает производительность на поддерживаемом оборудовании.

### CUDA

Koharu (Lilith Edition) собирается с поддержкой CUDA, что позволяет использовать мощность GPU NVIDIA для более быстрой обработки.

Koharu (Lilith Edition) включает CUDA toolkit 13.1 и cuDNN 9.19, динамические библиотеки будут автоматически извлечены в каталог данных приложения при первом запуске.
...
Koharu (Lilith Edition) поддерживает GPU NVIDIA с compute capability 7.5 или выше.
...
Вы всегда можете заставить Koharu (Lilith Edition) использовать CPU для инференса:
...
Koharu (Lilith Edition) использует несколько предобученных моделей для разных задач:
...
Модели будут автоматически загружены при первом запуске Koharu (Lilith Edition).
...
Для сборки Koharu (Lilith Edition) из исходников:
