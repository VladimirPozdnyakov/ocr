...
pub fn driver_version() -> Result<CudaDriverVersion> {
    ...
    // SAFETY: Loading NVIDIA driver library is safe if the driver is properly installed.
    // The function pointers (cuInit, cuDriverGetVersion) are standard CUDA driver API
    // functions that are stable across CUDA versions. We check the return values
    // to ensure the calls succeeded before using the results.
    unsafe {
...
pub(crate) fn is_available() -> bool {
    ...
    // SAFETY: Loading a library is safe as long as the library exists and is a valid shared library.
    // We only check if the library can be loaded (is_ok()), not actually use it.
